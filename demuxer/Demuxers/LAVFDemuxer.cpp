...
{
  return OpenInputStream(NULL, pszFileName, true);
}
...
STDMETHODIMP CLAVFDemuxer::OpenInputStream(AVIOContext *byteContext, LPCOLESTR pszFileName, bool openall)
{
  ...
  CHECK_HR(hr = InitAVFormat(pszFileName, openall));
...
STDMETHODIMP CLAVFDemuxer::InitAVFormat(LPCOLESTR pszFileName, bool openall)
{
  HRESULT hr = S_OK;

  const char *format = NULL;
  lavf_get_iformat_infos(m_avFormat->iformat, &format, NULL);
  if (!format || (!openall && !m_pSettings->IsFormatEnabled(format))) {
